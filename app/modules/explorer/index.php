 ...
 * ==========================================
 * Modified By Augusto Maza incorpered into Gesdoju Project
 * Was translate to Spanish Languaje
 * Added authentication by mysql/MariaDB version 10.3
 */
include "../../connection/connection.php";
include "../../../base_libs/lib_system.php";
    
    
    //$user = mysqli_real_escape_string($conn,$_POST["fm_usr"]);
	//$pass = mysqli_real_escape_string($conn,$_POST["fm_pwd"]);
	
	
	
	$user = $_SESSION['user'];
	$pass = $_SESSION['pass'];
	$_SESSION['fm_usr'] = $user;
	$_SESSION['fm_pwd'] = $pass;
	
	$sql = "select name from af_usuarios where user = '$user'";
  ...
  echo '</head><body style = "background: #839192;">';
	...
	echo '<a href="../../../logout.php"><hr><button type="buton" class="btn btn-default btn-block"><span class="glyphicon glyphicon-share" aria-hidden="true"></span> Iniciar</button></a>';	
	echo "</div></div>";
	...
	$sql = "SELECT * FROM af_usuarios where user = '$user' and password = '$pass' and state = 1";
	$q = mysqli_query($conn,$sql);
	
	$query = "SELECT * FROM af_usuarios where user = '$user' and password = '$pass' and state = 0";
...
$default_timezone = 'America/Argentina/Buenos_Aires'; // UTC+3
...
if (!@is_dir($root_path)) {
    echo sprintf('<h1>Carpeta Raíz "%s" no encontrado!</h1>', fm_enc($root_path));
    exit;
}
    ...
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        ...
        if (isset($auth_users[$_POST['fm_usr']]) && $_POST['fm_pwd'] === $auth_users[$_POST['fm_usr']]) {
            ...
            if($user = mysqli_fetch_assoc($retval)){
                ...
                unset($_SESSION['logged']);
                fm_set_msg('Usuario Bloqueado. Contacte al Administrador.', 'Error');
                fm_redirect(FM_SELF_URL);
				exit;
			}
			if($user = mysqli_fetch_assoc($q)){
                ...
                fm_set_msg('Bienvenido/a a Explorer ');
                fm_redirect(FM_SELF_URL . '?p=');
        
            }else{
            unset($_SESSION['logged']);
            fm_set_msg('Password Erronea', 'Error');
            fm_redirect(FM_SELF_URL);
        }
        }else{
        // Form
        unset($_SESSION['logged']);
        fm_show_header($nombre);
        fm_show_message();
        ?>
        <div class="path">
            <form action="" method="post" style="margin:10px;text-align:center">
                <input type="text" style="text-align: center" name="fm_usr" value="" placeholder="Usuario" required>
                <input type="password" style="text-align: center" name="fm_pwd" value="" placeholder="Password" required>
...
if (isset($_GET['del'])) {
    ...
    if ($del != '' && $del != '..' && $del != '.') {
        ...
        if (fm_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? 'Carpeta <b>%s</b> borrada' : 'Archivo <b>%s</b> borrado';
            fm_set_msg(sprintf($msg, fm_enc($del)));
        } else {
            $msg = $is_dir ? 'Carpeta <b>%s</b> no borrada' : 'Archivo <b>%s</b> no borrado';
            fm_set_msg(sprintf($msg, fm_enc($del)), 'error');
        }
    } else {
        fm_set_msg('Nombre de Archivo o Carpeta equivocado', 'error');
    }
...
if (isset($_GET['new'])) {
    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if (fm_mkdir($path . '/' . $new, false) === true) {
            fm_set_msg(sprintf('Carpeta <b>%s</b> creado', fm_enc($new)));
        } elseif (fm_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
            fm_set_msg(sprintf('Carpeta <b>%s</b> existente', fm_enc($new)), 'alert');
        } else {
            fm_set_msg(sprintf('Carpeta <b>%s</b> no creado', fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('Nombre de Carpeta Erroneo', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['copy'], $_GET['finish'])) {
    ...
    // empty path
    if ($copy == '') {
        fm_set_msg('Ruta no definida', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    // abs path from
    ...
    if ($from != $dest) {
        ...
        if ($move) {
            ...
            if ($rename) {
                fm_set_msg(sprintf('Movido desde <b>%s</b> a <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } elseif ($rename === null) {
                fm_set_msg('Archivo o Carpeta Existente en esta ubicación', 'alert');
            } else {
                fm_set_msg(sprintf('Error mientras se movia desde <b>%s</b> a <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        } else {
            if (fm_rcopy($from, $dest)) {
                fm_set_msg(sprintf('Copiado desde <b>%s</b> a <b>%s</b>', fm_enc($copy), fm_enc($msg_from)));
            } else {
                fm_set_msg(sprintf('Error mientras se copiadba desde <b>%s</b> a <b>%s</b>', fm_enc($copy), fm_enc($msg_from)), 'error');
            }
        }
    } else {
        fm_set_msg('Las rutas deben ser distintas', 'alert');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish'])) {
    ...
    if (!is_dir($copy_to_path)) {
        if (!fm_mkdir($copy_to_path, true)) {
            fm_set_msg('Imposible crear Carpeta de destino', 'error');
            fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
        }
    }
    // move?
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            $msg = $move ? 'Seleccione los Archivos o Carpetas a Mover' : 'Seleccione Archivos o Carpetas a copiar';
            fm_set_msg($msg);
        } else {
            $msg = $move ? 'Error mientras se movian los items' : 'Error while mientras se copiaban los items';
            fm_set_msg($msg, 'error');
        }
    } else {
        fm_set_msg('Nada Seleccionado', 'alert');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['ren'], $_GET['to'])) {
    ...
    if ($old != '' && $new != '') {
        if (fm_rename($path . '/' . $old, $path . '/' . $new)) {
            fm_set_msg(sprintf('Renombrar desde <b>%s</b> a <b>%s</b>', fm_enc($old), fm_enc($new)));
        } else {
            fm_set_msg(sprintf('Error minetras se renombraba desde <b>%s</b> a <b>%s</b>', fm_enc($old), fm_enc($new)), 'error');
        }
    } else {
        fm_set_msg('Nombres no seleccionados', 'error');
    }
    fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['dl'])) {
    ...
    if ($dl != '' && is_file($path . '/' . $dl)) {
        ...
        fm_set_msg('Archivo no Encontrado', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
}
...
if (isset($_POST['upl'])) {
    ...
    if ($errors == 0 && $uploads > 0) {
        fm_set_msg(sprintf('Todos los archivos subidos a <b>%s</b>', fm_enc($path)));
    } elseif ($errors == 0 && $uploads == 0) {
        fm_set_msg('Nada subido', 'alert');
    } else {
        fm_set_msg(sprintf('Error mientras se subían los archivos. Archvos Subidos: %s', $uploads), 'error');
    }
...
if (isset($_POST['group'], $_POST['delete'])) {
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            fm_set_msg('Seleccione Archivos o Carpetas a borrar');
        } else {
            fm_set_msg('Error minetras se borraban los items', 'error');
        }
    } else {
        fm_set_msg('Nada Seleccionado', 'alert');
    }
...
if (isset($_POST['group'], $_POST['zip'])) {
    ...
    if (!class_exists('ZipArchive')) {
        fm_set_msg('Operación con archivo no permitida', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    ...
    if (!empty($files)) {
        ...
        if ($res) {
            fm_set_msg(sprintf('Archivo <b>%s</b> creado', fm_enc($zipname)));
        } else {
            fm_set_msg('Archivo no creado', 'error');
        }
    } else {
        fm_set_msg('Nada Seleccionado', 'alert');
    }
...
if (isset($_GET['unzip'])) {
    ...
    if (!class_exists('ZipArchive')) {
        fm_set_msg('Operaciones con archivos no permitida', 'error');
        fm_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
    }
    ...
    if ($unzip != '' && is_file($path . '/' . $unzip)) {
        ...
        if ($res) {
            fm_set_msg('Archivo descomprmido');
        } else {
            fm_set_msg('Archivo no descomprimido', 'error');
        }
...
if (isset($_POST['chmod']) && !FM_IS_WIN) {
    ...
    if (@chmod($path . '/' . $file, $mode)) {
        fm_set_msg('Permisos cambiados');
    } else {
        fm_set_msg('Permisos no cambiados', 'error');
    }
...
if (isset($_GET['upload'])) {
    fm_show_header($nombre); // HEADER
    fm_show_nav_path(FM_PATH); // current path
    ?>
    <div class="path">
        <p><b>Subir Archivos</b></p>
        <p class="break-word">Carpeta de Destino: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?></p>
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <br>
            <p>
                <button class="btn"><i class="icon-apply"></i> Subir</button> &nbsp;
...
if (isset($_POST['copy'])) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            <p class="break-word">Carpeta Origen: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?><br>
            ...
            <p>
                <button class="btn"><i class="icon-apply"></i> Copiar</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> Cancelar</a></b>
            </p>
        </form>
    </div>
    <?php
...
if (isset($_GET['copy']) && !isset($_GET['finish'])) {
    ...
    <div class="path">
        <p><b>Copiando</b></p>
        <p class="break-word">
            Ruta: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . $copy)) ?><br>
            Carpeta de Destino: <?php echo fm_enc(fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH)) ?>
        </p>
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1"><i class="icon-apply"></i> Copy</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1&amp;move=1"><i class="icon-apply"></i> Mover</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> Cancelar</a></b>
        </p>
        <p><i>Seleccione Carpeta:</i></p>
...
if (isset($_GET['view'])) {
    ...
    <div class="path">
        ...
        <p class="break-word">
            ...
            Tamaño archivo: <?php echo fm_get_filesize($filesize) ?><?php if ($filesize >= 1000): ?> (<?php echo sprintf('%s bytes', $filesize) ?>)<?php endif; ?><br>
            ...
            if ($is_zip && $filenames !== false) {
                ...
                ?>
                Documentos en el archivo: <?php echo $total_files ?><br>
                Tamaño Total: <?php echo fm_get_filesize($total_uncomp) ?><br>
                Tamaño del Archivo: <?php echo fm_get_filesize($total_comp) ?><br>
        ...
        <p>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>"><i class="icon-download"></i> Descarga</a></b> &nbsp;
            <b><a href="<?php echo fm_enc($file_url) ?>" target="_blank"><i class="icon-chain"></i> Abrir</a></b> &nbsp;
            <?php
            // ZIP actions
            if ($is_zip && $filenames !== false) {
                $zip_name = pathinfo($file_path, PATHINFO_FILENAME);
                ?>
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>"><i class="icon-apply"></i> Descomprimir</a></b> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>&amp;tofolder=1" title="Unpack to <?php echo fm_enc($zip_name) ?>"><i class="icon-apply"></i>
                    Descomprimir a Carpeta</a></b> &nbsp;
                <?php
            }
            ?>
            <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-goback"></i> Volver</a></b>
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
            if ($filenames !== false) {
                ...
                echo '<p>Error mientras se accedia a la información</p>';
            }
        } elseif ($is_image) {
            // Image content
...
if (isset($_GET['chmod']) && !FM_IS_WIN) {
    ...
    ?>
    <div class="path">
        <p><b>Cambiar Permisos</b></p>
        <p>
            Ruta completa: <?php echo fm_enc($file_path) ?><br>
        </p>
        <form action="" method="post">
            <input type="hidden" name="p" value="<?php echo fm_enc(FM_PATH) ?>">
            ...
            <table class="compact-table">
                ...
                <tr>
                    <td style="text-align: right"><b>Lectura</b></td>
                    <td><label><input type="checkbox" name="ur" value="1"<?php echo ($mode & 00400) ? ' checked' : '' ?>></label></td>
                    <td><label><input type="checkbox" name="gr" value="1"<?php echo ($mode & 00040) ? ' checked' : '' ?>></label></td>
                    <td><label><input type="checkbox" name="or" value="1"<?php echo ($mode & 00004) ? ' checked' : '' ?>></label></td>
                </tr>
                <tr>
                    <td style="text-align: right"><b>Escritura</b></td>
                    <td><label><input type="checkbox" name="uw" value="1"<?php echo ($mode & 00200) ? ' checked' : '' ?>></label></td>
                    <td><label><input type="checkbox" name="gw" value="1"<?php echo ($mode & 00020) ? ' checked' : '' ?>></label></td>
                    <td><label><input type="checkbox" name="ow" value="1"<?php echo ($mode & 00002) ? ' checked' : '' ?>></label></td>
                </tr>
                <tr>
                    <td style="text-align: right"><b>Ejecución</b></td>
            ...
            <p>
                <button class="btn"><i class="icon-apply"></i> Cambiar</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="icon-cancel"></i> Cancelar</a></b>
            </p>
...
<table><tr>
<th style="width:3%"><label><input type="checkbox" title="Invertir selección" onclick="checkbox_toggle()"></label></th>
<th>Nombre Archivo/Carpeta</th><th style="width:10%">Tamaño</th>
<th style="width:12%">Fecha Creacion/Modificación</th>
<?php if (!FM_IS_WIN): ?><th style="width:6%">Permisos</th><th style="width:10%">Propietario</th><?php endif; ?>
...
<?php endif; ?>
<td>
<a title="Borrar" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Desea Eliminar?');"><i class="icon-cross"></i></a>
<a title="Renombrar" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i></a>
<a title="Copiar a..." href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="icon-copy"></i></a>
...
<tr>
<td><label><input type="checkbox" name="file[]" value="<?php echo fm_enc($f) ?>"></label></td>
<td><div class="filename"><a href="<?php echo fm_enc($filelink) ?>" title="Archivo Info"><i class="<?php echo $img ?>"></i> <?php echo fm_enc(fm_convert_win($f)) ?></a><?php echo ($is_link ? ' &rarr; <i>' . fm_enc(readlink($path . '/' . $f)) . '</i>' : '') ?></div></td>
...
<?php endif; ?>
<td>
<a title="Borrar" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Desea Borrar el Archivo?');"><i class="icon-cross"></i></a>
<a title="Renombrar" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="icon-rename"></i></a>
<a title="Copiar a..." href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="icon-copy"></i></a>
<a title="Direct link" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="icon-chain"></i></a>
<a title="Descargar" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="icon-download"></i></a>
</td></tr>
    <?php
    flush();
}
...
if (empty($folders) && empty($files)) {
    ?>
<tr><td></td><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><em>Carpeta Vacío</em></td></tr>
<?php
} else {
    ?>
<tr><td class="gray"></td><td class="gray" colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>">
Tamaño: <span title="<?php printf('%s bytes', $all_files_size) ?>"><?php echo fm_get_filesize($all_files_size) ?></span>,
...
<p class="path"><a href="#" onclick="select_all();return false;"><i class="icon-checkbox"></i> Seleccionar todo</a> &nbsp;
<a href="#" onclick="unselect_all();return false;"><i class="icon-checkbox_uncheck"></i> Deseleccionar todo</a> &nbsp;
<a href="#" onclick="invert_all();return false;"><i class="icon-checkbox_invert"></i> Invertir selección</a></p>
<p><input type="submit" name="delete" value="Borrar" onclick="return confirm('Desea Borrar los Archivos o Carpetas Seleccionados?')">
<input type="submit" name="zip" value="Comprimir" onclick="return confirm('Desea Comprimir?')">
<input type="submit" name="copy" value="Copiar"></p>
</form>
...
<a title="Subir Archivos" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="icon-upload"></i></a>
<a title="Crear Carpeta" href="#" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;"><i class="icon-folder_add"></i></a>
<?php if (FM_USE_AUTH): ?> 
<!-- <a title="Salir" href="../../logout.php"><i class="icon-logout"></i></a> -->
<!-- <a title="Salir" href="?logout=1"><i class="icon-logout"></i></a> -->
<?php endif; ?>
</div>
        <?php
        $path = fm_clean_path($path);
 ...
 * Show page header
 */
function fm_show_header($nombre)
{
...
a img{border:none}span.gray{color:#777}
...
tr:hover td.gray{background-color:#eee}
code,pre{
display:block;
margin-bottom:10px;font:13px/16px Consolas,'Courier New',Courier,monospace;
...
<div id="wrapper">
<hr>
<p><strong><small><img class="img-reponsive img-rounded" src="../../static/img/meeting-attending.png" /></span> Usuario: </strong><?php echo $nombre ?></p>
<hr>
<?php
}
...
{
    ?>
<hr><p class="center"><small><img class="img-reponsive img-rounded" src="../../static/img/user-home.png" /> Bienvenido/a al Explorer de <strong>Dirección Cloud</strong></small></p><hr>
</div>
<script>
function newfolder(p){var n=prompt('Nombre de la Carpeta','Carpeta');if(n!==null&&n!==''){window.location.search='p='+encodeURIComponent(p)+'&new='+encodeURIComponent(n);}}
